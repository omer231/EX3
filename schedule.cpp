...
namespace mtm {
    ...
    Schedule::~Schedule()
    {
        for (std::list<BaseEvent*>::iterator schedule_it = event_lst.begin(); schedule_it != event_lst.end(); ++schedule_it){
    ...
    {
    for(EventContainer::EventIterator container_it = event_container.begin(); container_it == event_container.end();++container_it)
        {   
            for (std::list<BaseEvent*>::iterator schedule_it = event_lst.begin(); schedule_it != event_lst.end(); ++schedule_it){
                mtm::BaseEvent& event = *container_it;
                if(event.getEventDate() == (*schedule_it)->getEventDate()&&event.getEventName() == (*schedule_it)->getEventName())
                {
    ...
    void Schedule::printAllEvents() const
    {
        ...
        event_filter_lst.sort( []( const mtm::BaseEvent *event1, const mtm::BaseEvent *event2 ) { return compare(event2,event1); } );
